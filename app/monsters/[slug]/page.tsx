import {Actions, Class, Monster, SpecialAbilities, STAT_ICONS} from "@/types";
import {CLASSES_API, fetchService, MONSTER_API} from "@/services/fetchService";
...
export default async function MonsterDetailPage({ params }: ClassDetailPageProps) {
    const { slug } = await params;
    const monster = await fetchService.getData<Monster>(MONSTER_API);
    const data = monster.find((c) => c.slug === slug);

    if (!data) {
        return (
            <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
                <div className="text-center">
                    <h1 className="text-4xl font-bold mb-4">Monster Not Found</h1>
                    <p className="text-slate-400">The Monster &quot;{slug}&quot; does not exist.</p>
                </div>
            </div>
        );
    }
